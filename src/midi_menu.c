...
static void add_store(int key, const struct desc *cmd) {
  ...
  char str_action[64];
  char *cp;
  snprintf(str_channel, sizeof(str_channel), "%d", cmd->channel);
  snprintf(str_note, sizeof(str_note), "%d", key);
...
void midi_menu(GtkWidget *parent) {
  ...
  gtk_header_bar_set_title(GTK_HEADER_BAR(headerbar), "piHPSDR - MIDI");
