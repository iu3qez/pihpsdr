...
int *g2panel_default_buttons(int andromeda_type) {
  ...
  case 5:
    ...
    result[22] = SPLIT;
    // 4x3 pad row 4 col 1, silk print: "10/F1", "no Band", default: selection_SNB
    result[23] = selection_SNB;
...
void g2panel_execute_button(int type, const int *vec, int button, int tr01, int tr10, int tr12, int tr20) {
  ...
  case BAND_PLUS:
  case BAND_MINUS:
    if (tr10) { schedule_action(action, MODE_PRESSED, 0); }

    if (tr12) { schedule_action(MENU_BAND, MODE_PRESSED, 0); }
  ...
  case FILTER_MINUS:
    ...
    if (tr12) { schedule_action(MENU_FILTER, MODE_PRESSED, 0); }
  ...
  case MODE_MINUS:
    ...
    if (tr12) { schedule_action(MENU_MODE, MODE_PRESSED, 0); }
  ...
  case NR:
    ...
    if (tr12) { schedule_action(MENU_NOISE, MODE_PRESSED, 0); }
  ...
  case TWO_TONE:
    ...
    if (tr12) { schedule_action(MENU_PS, MODE_PRESSED, 0); }

    break;

  default:
    if (tr01) { schedule_action(action, MODE_PRESSED, 0); }

    if (tr10 || tr20) { schedule_action(action, MODE_RELEASED, 0); }
...
void g2panel_execute_encoder(int type, const int *vec, int encoder, int val) {
  ...
  schedule_action(action, MODE_RELATIVE, val);
}
