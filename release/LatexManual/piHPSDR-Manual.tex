...
\end{figure}
User's Manual \\
\small{\pH post-2.5 development Version}
}
\author{
...
\textbf{\pH Version 2.6, under development}
\begin{itemize}
...
and automatically switched when changing the mode}
\end{itemize}
\medskip
...
or microphones to the host computer running \pH. Information about  compile-time options and how to add
...
have attached to the computer.  The possible choices are
...
\end{itemize}
Note that the front panel of the second-generation Anan G2 radios is not controlled via GPIO. For these
radios you should choose \rett{No Controller} or, even better, compile \pH without GPIO support.
...
to the panel. Note that even if you choose \rett{No Controller}, some GPIO lines may still be used
...
clock output (also 10 MHz). Sec. \ref{sec:hl2support} discusses the HL2-specific options
in more detail.
...
capabilities (ANF, NR1, NR2). Having the noise reduction before AGC normally works better,
but with sudden noise changes, having noise reduction after AGC may be preferred.
...
\rett{NR Settings} and \rett{NB Settings}.
You see a thin horizontal line in Fig. \ref{fig:NoiseMenu1}.
Choosing one of the two buttons just below the horizontal line determines whether the
...
\begin{itemize}
\item[RX latency]
{This is the time difference between a signal arriving at the antenna input, and the (demodulated) audio
playing in the headphone. It could be measured by connecting the (strongly attenuated) output of a
second transmitter to the antenna input, sending a stream of CW dahes, and monitoring both the output
of the second transmitter and the headphone of the first radio on a two-channel oscilloscope}
\item[TX latency]
...
\item[RX/TX turnover]
{This measures how fast the radio can switch from RX to TX. One way to measure is to feed the side tone
of a CW keyer producing three dashes and three dots to the microphone input while using VOX, and
measuring the microphone input and the RF output on a two-channel oscilloscope. Here not only the
delay (latency) is of interest but also to which extent the RX/TX transition "swallos" parts of the
first dahes sent.}
\item[TX/RX turnover]
{This measures how fast the radio can switch from TX to RX. It can be measured by turning the radio
into TX by shorting the PTT input to ground, and then releasing the PTT button. On a two-channel
oscilloscope one can measure the PTT and headphone output and see how long it takes to produce
a stable audio signal.}
\item[Audio latency]
{The delays mentioned above are only in part caused by the signal processing within \pH.
There is also a delay between \pH actually sending audio to the radio or to a soundcard,
...
rag-chew QSO, large delays (several hundred milliseconds) are not really harmful, but in a
...
about 33 ms if the distance is 10 thousand kilometers.
...
how fast the CPU is, but by how many ,,taps'' the signal processing chain contains. The number
...
menu) which corresponds to 43 ms at 48 kHz which is the (fixed) DSP rate in the RX engine. If
using additional processing (equalisers, noise reduction, etc.) the number of taps will be longer.
...
this should make underruns of the TX data buffer in the FPGA less likely.
...
increased on purpose to establish a resilience against small ,,hangs'' in the data flow,
...
sends key-up and key-down events via MIDI to the computer running \pH. Having low
latency in the \pH generated side tone is really important:
...
the third dot. Such a delay then leads to keying errors, e.g. sending a number '4' where
one wants to send a letter 'V'. Since the length of a dot is 50 ms at a CW speed of 24 wpm,
an audio delay of more than 100 ms is certainly not acceptable when doing CW. For the
PORTAUDIO and ALSA audio modules, \pH has a built-in mechanism to remedy this situation.

This is possible since when generating the CW side tone, audio samples come at a regular
and predictable pace, such that we do not need latency to cope with jitters in the audio
stream. Upon a RX/TX transition, most pending audio samples in the audio output buffer
are discarded such that the latency is small again. Furthermore, \pH very slighly
...
CW speeds up to 25 wpm. Unfortunately, this only applies to the ALSA and PORTAUDIO
audio module but not to PULSEAUDIO.
...
To demonstrate this, the side tone latence has been measured with a two-channel oscilloscope,
see Fig. \ref{fig:SideToneALSA}.
An external keyer was used that both sends MIDI key-up/down events to \pH and has a
hardware output (active low)
that was connected to channel 2 (blue trace)
of the oscilloscope and was used for triggering.
Channel 1 (yellow trace) then displays the signal at the headphone. Hitting the dot paddle then
produces a string of dots, a CW speed of 24 wpm (dot length 50 ms) was used.
For PulseAudio, the observed latency
was about 200 ms which makes this completely unusable for CW: when you hear the first dot,
the third dot is already being sent. For ALSA (see Fig. \ref{fig:SideToneALSA})
the measured delay is 30 ms, slightly more than half the dot length. This is not optimal
but still usable for doing CW at 24 wpm. On a Macintosh (using the PortAudio module), the
delay was even slightly smaller (20 ms). For 24 wpm, this is usable but not optimal, and
high-speed CW operators certainly want delays smaller than 10 ms.


\section{External solution to CW side tone latency}
Reducing the delay below 20 ms is certainly possible with dedicated low-latency audio
...
\pH. In my preferred setup, I use an external keyer built around a microcontroller, which
sends the key-up and key-down events via MIDI to the computer running \pH. You will find
lots of examples for microcontroller (uC) codes implementing a keyer on the internet, and for
...
signal, but the real problem is the audio clicks
that result from the sharp rise and fall of the side tone pulses. Both problems are eliminated
...
RX audio: the audio from the sound card goes into the keyer an the headphone is then
also plugged into the keyer.
Fig. \ref{fig:SideToneExternal} shows the measured result of such a setup. One immeadiately
...
center frequency of the audio band pass filter.
However, most CW operators have their preferred side tone frequency which does not change
...
(the author of this manual took part in this development). Briefly, the uC is so powerful
that it can run both a CW keyer software, and act as an external sound card to the
...
tuned to the TX frequency and mode while using \bltt{DUPLEX}: in most cases, the crosstalk at the T/R relay is
...
\section{The speech processor (CMPR, CRC)}
...
and are described in chapter \ref{sec:cfc}. Here we discusse the ,,normal'' speech processor,
...
\section{Additional controls in the \bltt{Radio} menu}
...
\end{figure}
 ...
 the audio codec on that board and includes RX audio samples in the data stream to the HL2.
...
because it contains a  microcontroller (a Raspberry Pico) with a lot
...
\textbf{Antenna Tuner.} When starting TUNE-ing, a '1' is written into register 7.
...
\section{HL2 parameters displayed in the panadapter}
...
are actually available, namely GPIO14/15for RadioBerries
 with firmware versions smaller than 73.2,
and GPIO17/18 for those with firmware versions 73.2 to 74.9, and GPIO 17/13 from firmware version 75.0 on.
...
\vskip0.2cm
and this will then switch your local repository to the release version 2.5. No more details on how
...
Older branches such as \texttt{Rel-2.3} are meanwhile outdated, the previous
release branch is named \texttt{Rel-2.4} and the current one, at the time of this writing, is \texttt{Rel-2.5}.
