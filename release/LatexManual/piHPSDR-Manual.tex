...
To demonstrate this, I have moved the Zoom slider from 1 to 32, such that Fig. \ref{noisefloorzoom} results.
...
we click the \bltt{CTUN} button (the third from the left in the row \rett{FNC(1)}) and
a   ,,command dialog'' pops up that looks as
...
\caption{Just selected \texttt{Duplex}.}
...
Note the scroll-bar at the right of this menu, since without scrolling it would be very tall.
...
button, that one gets highlighted (Fig. \ref{fig:ToolbarMenu3}).
...
in the line \rett{FNC(1)} has changed,
it now gives the short text (\texttt{DUP}) of the \bltt{Duplex} command. You can also see
that the \rett{FNC(1)} toolbar at the bottom of the screen has adopted the change, so
...
\rett{Button}. This type is generated by NoteOn/Off MIDI events. \pH commands (,,Actions'') that
...
\rett{Encoder}. This type is also generated by ControllerChange MIDI events. This means
...
you can choose between \rett{Encoder} and \rett{Knob/Slider}. In the example shown,
