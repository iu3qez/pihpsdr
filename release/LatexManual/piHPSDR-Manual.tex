...
is sent to the radio upon a RX/TX transition to pre-fill its TX data buffer. While not necessary
...
\subsection{CW side tone latency}
...
sends key-up and key-down events via MIDI to the computer running \pH. The audio latency
becomes an issue if \pH generates the side tone for you:
...
CW speeds up to 25 wpm. Unfortunately, this does not apply to the PULSEAUDIO module.

\subsection{External solution to CW side tone latency}
The author of this manual does a lot of CW, and therefore has thought a lot about the CW
side tone latency problem over the years. The most stringent solution to this problem is
...
tone at one of the uC output pins. This side tone is really ugly: not only the harmonics
contained in the square wave do not sound nice, the biggest problem is the audio clicks
that result from the sharp rise and fall of the side tone pulses. Both problems are eliminated
if one mangles the side tone through a narrow audio filter, that is easily realised with
a single OP amp. The only drawback is that the side tone then must have a fixed frequency.
However, most CW operators have their preferred side tone frequency which does not change
from one day to the other.
Using a simple AF transformer, the side tone can then be mixed with the RX audio. Simply
imagine that the AF transformer is inserted into the headphone cable.

The distinct advantage of this setup is, that your side tone follows, virtually without
latency, the state of the keyer. Of course, this setup implies that you set the side
tone volume to zero in the \bltt{CW} menu. However, you still want to have a side tone for
CAT-generated CW messages where latency is not an issue. To address this, \pH will use
...
from the keyer will be mixed into the RX audio stream in software.
...
\subsection{Monitoring the TX signal}
