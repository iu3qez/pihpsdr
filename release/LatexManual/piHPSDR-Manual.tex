...
This setting is part of the RX/TX profile (see Sec. \ref{sec:rxtxprofiles}). Most users
...
to \pH since it makes little difference to the application program. The (user) commands for
...
\texttt{pipewire-pulseaudio} which ensures that programs (such as \pH) using the
PulseAudio API can run with PipeWire. There are some subtle differences at command
